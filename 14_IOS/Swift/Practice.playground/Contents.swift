...
func calGrade(_ score: Int) -> String {
    switch score {
    case 90...100:
        return "A"
    case 80..<90:
...
func aggregation(names names: [String], scores scores: [Int]) {
    ...
    print("평균 점수: \(String(format:"%.2f", avg))")
    print("최고 점수: \(names[maxIdx]) (\(scores[maxIdx]))점")
    print("최저 점수: \(names[minIdx]) (\(scores[minIdx]))점")
}
...
func printGrade() {
    ...
    for i in 0..<names.count {
        print("\(names[i]): \(scores[i])점, 등급: \(calGrade(scores[i]))")
    }
}

printGrade()
print()
aggregation(names: names, scores: scores)
